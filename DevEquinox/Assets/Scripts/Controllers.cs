...
public abstract class Controllers : MonoBehaviour
{
    ...
    protected WaitForSeconds _updateRate;
    ...
    protected virtual void Awake()
    {
        ...
        _threshold = 0.01f;
        _updateRate = new WaitForSeconds(0.2f);
        ...
        _playerInput = GameObject.FindGameObjectWithTag(GlobalAccess._GameManager).GetComponent<PlayerInput>();
    ...
    {
        for (uint i = 0; i < Physics.OverlapSphereNonAlloc(transform.position, radius, _colliders, _enemiesLayer); i++) {
            if (_colliders[i] != null) {
                if (_colliders[i].TryGetComponent(out EnemyAI AI)) AI.OnAware(transform);
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if(_triggetStayTimeout > GlobalAccess._triggerUpdateRate) {
            ...
            if (other.TryGetComponent(out EnemyAI AI)) AI.OnAware(transform);
